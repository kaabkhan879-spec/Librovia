...
export const BookDetailsPage: React.FC = () => {
  const { refreshSubscription, currentPlanId } = useSubscription()
  ...
  const handleShareBook = async () => {
    if (!rawBook || !resolvedRecipient) return
    try {
      if (resolvedRecipient.plan === 'FREE') {
        showError('Free users cannot receive shared books')
        return
      }

      await sharesService.shareBook(rawBook.id, resolvedRecipient.email, currentUser?.email || '')
      showSuccess(`Book successfully shared with ${resolvedRecipient.name}! 📤`)
      setIsShareModalOpen(false)
      setShareEmail('')
      setResolvedRecipient(null)
    } catch (err) {
      console.error(err)
      const msg = err instanceof Error ? err.message : 'Failed to share book'
      showError(msg)
    }
  }

  const handleRemoveFromLibrary = async () => {
    if (!rawBook || !currentUser) return
    if (!confirm('Are you sure you want to remove this shared book from your library?')) return
    setIsDeleting(true)
    try {
      const shares = await sharesService.getSharedWithMe()
      const currentShare = shares.find((s) => s.book_id === rawBook.id)
      if (currentShare) {
        await sharesService.removeShare(currentShare.id, currentUser.id)
        showInfo(`Removed "${rawBook.title}" from your library.`)
        navigate(ROUTES.LIBRARY)
      } else {
        showError('Sharing record not found.')
      }
    } catch (e) {
      console.error(e)
            ...
            <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {isShared ? (
                      <button
                        ...
                        className="dark:border-slate-750 flex h-10 cursor-pointer items-center justify-center gap-1.5 rounded-xl border border-red-200 bg-red-50 text-[10px] font-bold tracking-wider text-red-600 uppercase hover:bg-red-100/50 dark:bg-red-950/20 dark:text-red-400"
              ...
              <div className="space-y-2 rounded-3xl border border-slate-100 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  {!isShared && (currentPlanId === 'pro' || currentPlanId === 'family') && (
                    <button
                  ...
                  {!isShared && (
                    <button
                      ...
                      className="text-red-655 col-span-2 flex h-9 cursor-pointer items-center justify-center rounded-xl border border-red-200 bg-red-50 text-[10px] font-bold tracking-wider uppercase hover:bg-red-100 disabled:opacity-50"
                    >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-4">
                ...
                {/* Recipient Search Results Canvas */}
                {searchError && (
                  <div className="rounded-2xl border border-rose-100 bg-rose-50/50 p-4 text-center text-xs font-bold text-rose-600 dark:border-rose-950/20 dark:bg-rose-950/20 dark:text-rose-400">
                    ❌ {searchError}
                  </div>
                )}
                ...
                {resolvedRecipient && (
                  <div className="rounded-2xl border border-slate-100 bg-slate-50/40 p-4 space-y-4 text-left dark:border-slate-850 dark:bg-slate-800/20">
                        ...
                        <div className="flex items-center justify-between">
                          <label className="text-[10.5px] font-bold text-slate-600 dark:text-slate-350 flex items-center gap-2">
                            <input
                              ...
                              checked={true}
                              disabled={true}
                              className="h-3.5 w-3.5 rounded border-slate-300 text-purple-600 accent-purple-600 focus:ring-purple-500"
                            />
                            <span>Read Only</span>
                          </label>
                    ...
                    {resolvedRecipient.plan !== 'FREE' ? (
                      ...
                      <div className="rounded-2xl bg-slate-100 p-3 text-center text-[10px] font-bold text-slate-400 dark:bg-slate-800">
                        ⚠️ Free users cannot receive shared books.
                      </div>
                    )}
                  </div>
                )}
